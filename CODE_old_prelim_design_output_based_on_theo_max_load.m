...
load("prelimDesignOptimize2_61oz.mat")
...
format short
units = "oz";
disp("\\% EK301, Section A1, Group 1: Aadarsh A., Peter Z., Jilin Z. 4/20/2023.")
% fprintf("Load: %i oz\n", appliedLoad)
...
% test
% disp("Strong Load: ")
WfailureStrong = (T(critMember)+Ufit)/Rm(critMember);
% disp("Weak Load: ")
WfailureWeak = (T(critMember)-Ufit)/Rm(critMember);

% disp("Original Method:")
% disp("")
% % STRONG CASE
% condition = true;
% critMember = 0;
...
%% Failure Output
fprintf("Critical member: %d\n",critMember)
WfailureDiffHalved = (WfailureStrong - WfailureWeak) / 2.0;
fprintf("Theoretical max load is: %f +/- %f \n", ...
    WfailureDiffHalved+WfailureWeak,WfailureDiffHalved)
%% Function(s)
